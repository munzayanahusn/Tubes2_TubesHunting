...
namespace BFSalgorithm
{
    class BFS : PlayerAction
    {
        ...
        public void breadthFirstSearch(PlayerGame.Position pos, Maze maze, Game.GameState game, int treasureFound)
        {
            int[] dx = { 0, 1, 0, -1 };
            int[] dy = { -1, 0, 1, 0 };
            ...
            if (game.getTreasureCount() > 0)
            {
                while (q.Count > 0)
                {
                    ...
                    PlayerGame.Position currentPos = new PlayerGame.Position(current.x, current.y);
                    setCurrentPosition(currentPos);
                    this.setRoute(current.route);
                    int steps = current.steps;

                    if (maze.getMapElement(currentPos.getX(), currentPos.getY()) == Game.GameState.TREASURE_PLACE)
                    {
                        maze.setMapElement(Game.GameState.ROAD, currentPos.getX(), currentPos.getY());
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        if (maze.getMapElement(newX, newY) == Game.GameState.OBSTACLES
